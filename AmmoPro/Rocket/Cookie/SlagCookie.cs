...
{
    public class SlagCookie : ModProjectile
    {
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            for (int j = 0; j < 2; j++)
            {
                Dust d = Dust.NewDustDirect(Projectile.position, Projectile.width, Projectile.height, 15, 0f, 0f, 100, default, 3f);
                d.noGravity = true;
                d.velocity *= 4f;
            }
        }
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            Projectile.NewProjectile(Projectile.InheritSource(Projectile), Projectile.Center, v, ModContent.ProjectileType<CookieBoom>(), Projectile.damage * 3, Projectile.knockBack = 0, Projectile.owner);
            for (int i = 0; i < 4; i++)
            {
                Vector2 v2 = new Vector2(Main.rand.NextFloat(-3, 3), Main.rand.NextFloat(-10, -8));
                Projectile child = Projectile.NewProjectileDirect(Projectile.GetSource_FromThis(), Projectile.Center, v2 * 2, ModContent.ProjectileType<CookieResidue>(), Projectile.damage / 2, Projectile.knockBack, Main.myPlayer, 0, 1);
            }
            for (int j = 0; j < 2; j++)
            {
