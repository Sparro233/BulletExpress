...
{
    public class CursedFlameGrenade : ModProjectile
    {
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                Vector2 v = new Vector2(Main.rand.NextFloat(-5, 5), Main.rand.NextFloat(5, -5));
                Projectile child = Projectile.NewProjectileDirect(Projectile.GetSource_FromThis(), Projectile.Center, v, ProjectileID.CursedFlameFriendly, Projectile.damage / 2, Projectile.knockBack, Main.myPlayer, 0, 1);
            }
            for (int j = 0; j < 8; j++)
            {
                Dust d = Dust.NewDustDirect(Projectile.position, Projectile.width, Projectile.height, DustID.CursedTorch, 0f, 0f, 100, default, 1.5f);
                ...
                Dust dI = Dust.NewDustDirect(Projectile.position, Projectile.width, Projectile.height, DustID.CursedTorch, 0f, 0f, 100, default, 2.5f);
