...
{
    public class StormLetterOpener : ModProjectile
    {
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            for (int j = 0; j < 1; j++)
            {
                ...
                Projectile.NewProjectile(Projectile.GetSource_OnHit(target), target.Center - vector2 * 20f, vector2, ProjectileType<StormTear>(), Projectile.damage / 2, 0f, Projectile.owner, 0f, target.Center.Y);
            }
        }
        ...
        public override void OnKill(int timeLeft)
        {
            for (int j = 0; j < 6; j++)
            {
                Dust d = Dust.NewDustDirect(Projectile.position, Projectile.width, Projectile.height, ModContent.DustType<IDA.Powders.LightIV>(), 0f, 0f, 15, default, 1f);
                d.velocity *= 1f;
            }
