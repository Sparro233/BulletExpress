namespace BulletExpress.IDA.Tiles
{
    ...
    public class AgarFlowers : ModTile
    {
        //用于可读性和踢出界的常量
        ...
        public override bool CanPlace(int i, int j)
        {
            ...
            if (tile.HasTile)
            {
                ...
                if (tileType == Type)
                {
                    PSII stage = GetStage(i, j); 
                    //草药的当前阶段
                
                    return stage == PSII.Grown;
        ...
        public override IEnumerable<Item> GetItemDrops(int i, int j)
        {
            ...
            int herbItemType = ModContent.ItemType<IDB.Materials.AgarDew>();
            int herbItemStack = 1;

            int seedItemType = ModContent.ItemType<IDB.Materials.WheatSeed>();
            int seedItemStack = 1;

            if (stage == PSII.Growing)
            {
                //成熟
                herbItemStack = 1;
            }
            ...
            /*if (stage == PSII.Grown && nearestPlayer.active && (nearestPlayer.HeldItem.type == ItemID.StaffofRegrowth || nearestPlayer.HeldItem.type == ItemID.AcornAxe))
            {
                //通过再生员工提高产量
                herbItemStack = Main.rand.Next(1, 3);
                seedItemStack = Main.rand.Next(1, 6);
        ...
        private static PSII GetStage(int i, int j)
        {
            Tile tile = Framing.GetTileSafely(i, j);
            return (PSII)(tile.TileFrameX / FrameWidth);
        }
    }
}